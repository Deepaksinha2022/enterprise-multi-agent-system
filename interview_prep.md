...
Why do we keep both search() and research() functions in the ResearchAgent instead of putting everything inside a single function?

We separate search() and research() to follow the Single Responsibility Principle. search() is responsible only for interacting with the search API, while research() orchestrates the complete research workflow, such as calling search(), filtering results, summarizing them, structuring the output, and returning it to the Supervisor. This reduces coupling, improves cohesion, makes the code easier to maintain and test, and allows us to replace the search provider without changing the interface exposed by the ResearchAgent.

---------------------------------------------------------------

Question 2 ⭐⭐⭐⭐⭐
Why do we structure Tavily's response before returning it to the Supervisor instead of returning the raw API response?

We structure Tavily's response before returning it to the Supervisor to maintain a stable and consistent interface. The Supervisor should not depend on Tavily's raw response format because if the API response changes or we switch to another search provider like Google or Bing, the Supervisor would break. By restructuring the response into a standardized format (e.g., title, summary, source), we achieve loose coupling, high cohesion, maintainability, and provider independence. Any API-specific changes remain confined to the ResearchAgent, while the Supervisor continues to consume the same predictable output structure.

-------------------------------------------------------------

Question 3 ⭐⭐⭐⭐⭐
Suppose tomorrow you replace Tavily with Google Search API.
Which file(s) would you modify, and which file(s) would remain unchanged?

Only the ResearchAgent needs to be modified, specifically the search() function where the API call is made. The research() function continues to return the same standardized output (title, summary, source), so the Supervisor and all other components remain unchanged. This design follows loose coupling and abstraction principles—the API-specific logic is isolated inside search(), while the rest of the system depends only on the stable interface exposed by the ResearchAgent.
...
Explain the complete flow from the moment the query enters the ResearchAgent until the structured result is returned to the Supervisor.

Mention the major functions and data transformations only.

The user query is received by the ResearchAgent, which calls the search() function. The search() function sends the query to the Tavily API and receives the raw search response. The response is then returned to the research() function, where the results are restructured into a standardized format (title, summary, source). A final consolidated summary is generated by combining the individual summaries. Finally, the ResearchAgent returns a structured response containing the query, summary, and sources to the Supervisor.

--------------------------------------------------------------

Question 5 ⭐⭐⭐⭐
Why do we use an environment variable (TAVILY_API_KEY) instead of hardcoding the API key in the code?

API keys should never be hardcoded because they expose sensitive credentials and can leak through source code or version control. Storing the key in a .env file improves security and follows the principle of separating configuration from code. It also improves maintainability, since updating the API key only requires changing the .env file rather than modifying the source code. This approach also allows different environments (development, testing, production) to use different API keys without changing the application code.

-------------------------------------------------------------

Question 6 ⭐⭐⭐⭐⭐
Why did we return only title, summary, and source from each search result instead of returning all the fields provided by Tavily (such as score, raw_content, images, etc.)?

We return only title, summary, and source because these are the fields required by the Supervisor for reasoning and response generation. Returning unnecessary fields such as score, images, or raw_content increases coupling and exposes API-specific details. By keeping only the required fields, we create a clean, predictable, and provider-independent interface. If additional fields are needed in the future, they can be added without affecting the overall architecture.

-------------------------------------------------------------

Question 7 ⭐⭐⭐⭐⭐ (Last)
Suppose Tavily API is down or returns an error.
Where should you handle this exception—in the Supervisor or in the ResearchAgent? Why?
